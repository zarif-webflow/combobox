...
const PROPERTIES = {
  comboboxMargin: 'data-input-combobox-margin',
} as const;
...
const setupCombobox = (comboboxInput: HTMLComboboxInputElement, initialResults: string[] = []) => {
  ...
  const comboboxMargin = comboboxInput.getAttribute(PROPERTIES.comboboxMargin) || '0.5rem';
  ...
  const setModalPosition = () => {
    computePosition(comboboxInput, resultContainer, {
      ...
      setStyle(resultContainer, {
        left: `${x}px`,
        top: `${y}px`,
        transform: placement.includes('bottom')
          ? `translateY(${comboboxMargin})`
          : `translateY(calc(-1 * ${comboboxMargin}))`,
      });
    });
  };
